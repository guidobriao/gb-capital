...
# -*- coding: utf-8 -*-
"""
Free Cash Flow Forecasting with SARIMAX (pmdarima)
Annual FCF prediction using Quarterly FCF as lagged exogenous variables

This model uses pmdarima's auto_arima with exogenous regressors (X).
...
def prepare_data(quarterly_df: pd.DataFrame, annual_df: pd.DataFrame, test_years: int = 4):
    """Prepare data for SARIMAX training.
...
def train_sarimax_model(
    ...
    """Train SARIMAX model with optional exogenous variables.

    Uses pmdarima's auto_arima to find best ARIMA parameters,
    then forecasts with exogenous variables if available.
    ...
    """
    logger.info("Training SARIMAX model with pmdarima...")
    ...
    logger.info("Running auto_arima to find best parameters...")
    ...
    if use_exog and train_X_clean is not None:
        auto_arima_params['X'] = train_X_clean
    
    model = auto_arima(**auto_arima_params)
    
    logger.info(f"Best model: ARIMA{model.order}")
...
def save_predictions(predictions: np.ndarray, actuals: np.ndarray, ticker: str, fy_mapping: dict, train_size: int):
    ...
    output_file = RESULT_DIR / f"{ticker}_fcf_annual_predictions_autoarima.csv"
...
def train_and_predict(ticker: str, test_years: int = 4):
    ...
    """
    logger.info(f"\n{'=' * 80}")
    logger.info(f"Starting FCF forecasting pipeline for {ticker} (SARIMAX)")
    logger.info(f"{'=' * 80}\n")
    ...
    model, predictions, y_scaler, X_scaler = train_sarimax_model(
